...
namespace soap_fuzzer
{
    class MainClass
    {
        ...
        public static void Main(string[] args)
        {
            ...
            HttpWebRequest req = (HttpWebRequest)WebRequest.CreateHttp(_endpoint + "?WSDL");
        ...
        static void FuzzHttpGetPort(SoapBinding binding)
        {
            ...
            foreach (SoapBindingOperation op in binding.Operations)
            {
                ...
                foreach (Guid guid in guidList)
                {
                    ...
                    HttpWebRequest req = (HttpWebRequest)WebRequest.CreateHttp(testUrl);
        ...
        static void FuzzSoapPort(SoapBinding binding)
        {
            ...
            foreach (SoapBindingOperation op in binding.Operations)
            {
                Console.WriteLine("Fuzzing operations: " + op.Name);
                SoapOperation po = portType.Operations.Single(p => p.Name == op.Name);
                SoapMessage input = _wsdl.Messages.Single(m => m.Name == po.Input.Split(':')[1]);
                ...
                foreach (Guid parm in paramList)
                {
                    ...
                    HttpWebRequest req = (HttpWebRequest)WebRequest.CreateHttp(_endpoint);
                    ...
                    catch (WebException ex)
                    {
                        ...
                        if (resp.Contains("syntax error"))
                            Console.WriteLine("Possible SQL injectionv ector in parameter: ");
